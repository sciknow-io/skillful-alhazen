...
const GRANULARITY_COLORS: Record<string, string> = {
  'single-command':        'bg-green-500/20 text-green-400 border-green-500/30',
  'multi-step-sequential': 'bg-blue-500/20 text-blue-400 border-blue-500/30',
  'configuration-driven':  'bg-purple-500/20 text-purple-400 border-purple-500/30',
  'programmatic-api':      'bg-orange-500/20 text-orange-400 border-orange-500/30',
  'interactive-notebook':  'bg-yellow-500/20 text-yellow-400 border-yellow-500/30',
};

export function GranularityBadge({ granularity }: { granularity?: string }) {
  return (
    <Badge className={getBadgeColor(GRANULARITY_COLORS, granularity)}>
      {granularity ?? 'unknown'}
    </Badge>
  );
}
