...
import { DomainCard } from '@/components/domain-modeling/domain-card';
import type { DomainSummary } from '@/components/domain-modeling/domain-card';
...
export default function DomainModelingPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch('/api/domain-modeling/domains');
      ...
      <main className="container mx-auto px-4 py-6">
        {error && (
          <div className="bg-destructive/10 text-destructive px-4 py-3 rounded-lg mb-6">
            ...
            <p className="text-sm mt-1">Make sure TypeDB is running and the domain-modeling skill is configured.</p>
          </div>
        )}
